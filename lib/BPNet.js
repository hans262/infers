...
class BPNet {
    ...
    forwardPropagation(xs) {
        ...
        for (let l = 0; l < this.hlayer; l++) {
            ...
            hy[l] = tmp.atomicOperation((item, i) => common_1.afn(item, tmp.getRow(i), af));
        }
        return hy;
    }
    scaled(xs) {
    ...
    backPropagation(hy, xs, ys) {
        let m = ys.shape[0];
        if (m === 1)
            return this.calcBackPropagation(hy, xs, ys);
    ...
    calcBackPropagation(hy, xs, ys) {
        let dw = [], dy = [];
        for (let l = this.hlayer - 1; l >= 0; l--) {
            let lastHy = hy[l - 1] ? hy[l - 1] : xs;
            let af = this.af(l);
            if (l === this.hlayer - 1) {
                dy[l] = hy[l].atomicOperation((item, r, c) => (item - ys.get(r, c)) * common_1.afd(item, af));
            }
            else {
                dy[l] = dy[l + 1].multiply(this.w[l + 1]).atomicOperation((item, r, c) => item * common_1.afd(hy[l].get(r, c), af));
            }
